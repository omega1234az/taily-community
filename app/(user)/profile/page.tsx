    ...
    <div>
      ...
      <div className="ml-5 sm:ml-10 md:ml-10 lg:mr-10 xl:ml-14 2xl:ml-20 mt-6 flex flex-col gap-6 sm:text-md xl:text-xl">
        <div className="lg:flex justify-between gap-6 md:w-full">
          <InputField label="ชื่อ" value={formData.firstName} onChange={handleChange("firstName")} disabled={!isEditing || isSaving} />
          <InputField label="สกุล" value={formData.lastName} onChange={handleChange("lastName")} disabled={!isEditing || isSaving} />
        </div>
        <div className="lg:flex justify-between gap-6 md:w-full">
          <InputField label="อีเมล" type="email" value={formData.email} onChange={handleChange("email")} disabled={true} /> {/* email ไม่ให้แก้ไข */}
          <InputField label="เบอร์โทร" type="tel" value={formData.phone} onChange={handleChange("phone")} disabled={!isEditing || isSaving} />
        </div>
      </div>
      ...
      <div className="ml-5 sm:ml-10 md:ml-10 lg:mr-10 xl:ml-14 2xl:ml-20 mt-6 flex flex-col gap-4 sm:text-md xl:text-xl">
        <div className="lg:flex justify-between gap-6 md:w-full">
          <InputField label="บ้านเลขที่" value={formData.houseNumber} onChange={handleChange("houseNumber")} disabled={!isEditing || isSaving} />
          <InputField label="ถนน" value={formData.street} onChange={handleChange("street")} disabled={!isEditing || isSaving} />
        </div>
        <div className="lg:flex justify-between gap-6 md:w-full">
          <InputField label="หมู่" value={formData.village} onChange={handleChange("village")} disabled={!isEditing || isSaving} />
          <InputField label="ตำบล" value={formData.subDistrict} onChange={handleChange("subDistrict")} disabled={!isEditing || isSaving} />
        </div>
        <div className="lg:flex justify-between gap-6 md:w-full">
          <InputField label="อำเภอ" value={formData.district} onChange={handleChange("district")} disabled={!isEditing || isSaving} />
          <InputField label="จังหวัด" value={formData.province} onChange={handleChange("province")} disabled={!isEditing || isSaving} />
        </div>
      </div>
